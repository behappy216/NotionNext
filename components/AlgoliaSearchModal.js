        ...
        <ul className='flex-1 overflow-auto'>
          {searchResults.map((result, index) => (
            <li
              ...
              rounded-lg
              ${activeIndex === index ? 'bg-blue-600 dark:bg-grey-600' : ''}`}>
              <a
      ...
      {firstTenTags?.map((tag, index) => {
        return (
          <Link
            ...
            <div
              className={
                'flex items-center text-black dark:text-gray-300 hover:bg-blue-600 dark:hover:bg-grey-600 hover:scale-110 hover:text-white rounded-lg px-2 py-0.5 duration-150 transition-all'
              }>
  ...
  return (
    <div className='flex space-x-1 w-full justify-center py-1'>
      {Array.from({ length: totalPage }, (_, i) => {
        const classNames =
          page === i
            ? 'font-bold text-white bg-blue-600 dark:bg-grey-600 rounded'
